...
!!document.querySelector(".find-influencer") &&
(function () {
  let $fixedNavHeight, $sectionsWrapperOffset, msnry;
  ...
  $singleInflModal.on("scroll", function (e) {
  ...
  .on("show.bs.modal", ".modal-single--influencer", function (e) {
    let $thisCard = jQuery(e.relatedTarget).closest(
      ".single-profile--card"
    );
  ...
  .on(
    "hide.bs.dropdown",
    ".keepDropdownOpen .dropdown, .modal-open .dropdown ",
    function (e) {
      e.preventDefault();
    }
  )
  .on("shown.bs.tab", function (e) {
    jQuery(jQuery(e.target).attr("href") + " .owl-carousel").resize();
  })
  ...
  .on("click", ".single-card--infobtn", function (e) {
    let _profile = jQuery(e.currentTarget).parents(".single-profile--card"),
      _slider = _profile.find(".owl-carousel");

    _slider.trigger(
      "to.owl.carousel",
      _slider.data("owl.carousel")._items.length - 1
    );
  })
  ...
  $(".badge-outline--dark,.single-infl--gscore").tooltip();
  ...
  $(".single-card--fixednav .btn-close").on("click", function (e) {
  ...
  jQuery(document).on(
    ...
    function (e) {
      ...
      let $parent = jQuery(e.target).closest("[class*=collapse-]");
  ...
  function owlFix(owl) {
    const $parentEl = owl.relatedTarget.$element.closest(
      ".single-card--owlwrapper"
    );
    const targetW = Math.trunc(
      owl.relatedTarget.$element.closest(".single-profile--card").width()
    );
