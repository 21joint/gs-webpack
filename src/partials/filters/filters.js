...
const Filters = (() => {
  ...
  $(".search-filter--ul .dropdown")
    ...
    .each(function(i, el) {
      ...
      _filter.$applyBtn.on("click", function(e) {
        ...
        _filter.$toggler.addClass(
          _filter.$valueEl.text().length > 0 ? "active" : ""
        );
        _filter.$toggler.is(".active") &&
          !_filter.$valueEl.text().length &&
          _filter.$toggler.removeClass("active");
      });
  ...
  $(".search-filter--ul .dropdown-menu").on("click", ".btn-cancel", e => {
    $(e.target)
      .parents("li")
  ...
  $("[title=gs-handle--input]").on("keyup", e => {
    if (e.keyCode === 38) {
      e.target.value++;
    }
    if (e.keyCode === 40) {
