...
class Influencer {
  ...
  createSlides() {
    ...
    for (let j = 0; j < this.photos.length; j++) {
      ...
      _slides = _slides + `<div class="single-card--slide">
        <a role="button" tabindex="1" data-toggle="modal" data-target=".modal-single--influencer">
          <div class="single-slide--image" style="background-image:url(${ photo });"></div>
        </a>
      </div>`;
    }
    return _slides;
  ...
  static createSocial(instance) {
    let _s = '', _quantityHandler;
    ...
    for (let key in instance.social) {
      if (instance.social.hasOwnProperty(key)) {
        _s += '<li>' + _Icons.insertIcon({title: key}) + '<span class="text-asphalt">' + _quantityHandler(instance.social[key]) + 'k</span></li>'
      }
    }
    return _s;
  }
  ...
  init() {
    ...
    <a role="button" class="single-card--likebtn">
    ...
    <div class="single-card--title">
      <div class="row justify-content-between flex-nowrap align-items-center">
        ...
        <div class="col-auto">
          <button type="button" class="single-card--infobtn w-100 bg-white border-0 p-0 text-right">
            <img src="${require('../assets/images/info.svg')}" class="r-icon--medium align-middle ml-1" alt=" ">
          </button>
        </div>
      </div>
    </div>
    <div class="single-card--owlwrapper">
      <div class="single-card--carousel owl-carousel">
        ${ this.createSlides() }
        <div class="single-card--slide">
          <div class="single-slide--more">
            <div class="container-fluid h-100 px-3">
              <div class="row align-items-center justify-content-center h-100">
                <div class="col-12">
                  <h3>LifeStyle, Parenting</h3>
                  <p>Pueraria Mirifica And Study Phyto Estrogens. Answer: Unless your complexion is very oily, your doctor is right: </p>
                  <div>
                    <button class="btn btn-sm btn-primary" data-toggle="modal" data-target=".modal-single--influencer">
                      See More &amp; Contact
                    </button>
                  </div>
                </div>
    ...
    <div class="single-card--networks">
      <ul class="flex-row">${Influencer.createSocial(_self)}</ul>
    </div>
</div>`;
  }
}
